...
begin
  ...
  architecture arch_FD of FD is
      ...
      signal one: bit_vector(8 downto 0);
      begin
          ...
          a_in <= a_out when (s1 = '0') else
                  bit_vector((unsigned(a0) + unsigned(a_out)));

          b_in <= b_out when (s2 = '0') else
                  bit_vector((unsigned(b0) + unsigned(b_out)));
          sum <= nSomas_out when (s1 = '0' and s2 = '0') else
                          bit_vector((unsigned(nSomas_out) + unsigned(one)));
  ...
  architecture arch_MMC16 of mmc is
      ...
      begin
          ...
          xFD: FD
          port map(clock_n, h1, h2, s1, s2, x1, diferente, maior, A, B, inicia, nSomas, mmc);
