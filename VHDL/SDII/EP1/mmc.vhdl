...
begin
  ...
  architecture arch_FD of FD is
      ...
      signal a_sig, b_sig: bit_vector(7 downto 0);
      signal a_in, b_in: bit_vector(7 downto 0); -- Fio que liga o MUX ao REG
      ...
      begin
          ...
          regCMD: register16
          port map (clock, '0', x1, a, mmc);

          a_in <= A when (s2 = '0') else
                  bit_vector((unsigned(A) + unsigned(a_sig)));

          b_in <= B when (s1 = '0') else
                  bit_vector((unsigned(B) + unsigned(b_sig)));
  ...
  architecture arch_MMC16 of mmc16 is
      component UC is
          port(
              -- Sinais de controle global
              clock, reset: in bit;
