%%%% ///////////////////////////////////////////////////////////////////////////
%%%%        PTC3213 - EC1 - 2021 - DIFEREN�AS FINITAS
%%%%
%%%%    11806851    Felipe Dutra Teixeira (Substitua pelos num. e nomes corretos em ordem alf.)
%%%%    NUSP2 Aluno B
%%%%    NUSP3 Aluno C
...
%%%   Dados de entrada
%%%
NUSP = 11806851  ; % NUSP do 1o aluno (ordem alfab.)
%
...
%%%   Atualiza o potencial dos nos internos pela media dos 4 vizinhos - Eq. Laplace - M.D.F.
    for k=1:size(p,1);
        [i,j]=ind2sub(size(x),p(k));
            Phi_new(i,j)=(Phi_new(i-1,j)+Phi_new(i+1,j)+Phi_new(i,j-1)+Phi_new(i,j+1))/4; % Eq. Laplace - M.D.F.
    end
...
titulo=['Mapa de Quadrados Curvilineos (EC1 2022)  ', date()];
title(titulo);
hold off
%%%
